...
describe('Watchlist Service - Benchmarks', () => {
  ...
  describe('Add to Watchlist', () => {
    it('should add stock within threshold', async () => {
      const result = await runner.run(
        'watchlist-add',
        async () => {
          await service.addStock('test-user', '600519.A', {
            notes: 'Test stock',
            targetPrice: 2000,
          });
        },
      ...
      const result = await runner.run(
        ...
        async () => {
          for (const stock of stocks) {
            await service.addStock('test-user', stock.symbol, { notes: stock.notes });
          }
        },
        {
          maxTime: 200,
  ...
  describe('Get Watchlist', () => {
    it('should get watchlist within threshold', async () => {
      ...
      await service.addStock('test-user', '600519.A');
      await service.addStock('test-user', '000001.A');

      const result = await runner.run(
        'watchlist-get',
        async () => {
          await service.getWatchlist('test-user');
        },
  ...
  describe('Update Watchlist Item', () => {
    it('should update item within threshold', async () => {
      ...
      const result = await runner.run(
        'watchlist-update',
        async () => {
          await service.updateStock('test-user', '600519.A', {
            notes: 'Updated notes',
            targetPrice: 2100,
          });
        },
  ...
  describe('Memory Efficiency', () => {
    it('should not leak memory during operations', async () => {
      ...
      const { result, delta } = await measureMemory(async () => {
        for (let i = 0; i < 100; i++) {
          await service.addStock('test-user', `600${String(i).padStart(3, '0')}.A`);
        }
      });
